...
workflow) see [README.md](README.md); for positioning against other languages
see [comparison.md](comparison.md). This file is the cached, precise
description of how the codebase works, plus the design intent behind it
...
So the working surface is the full pipeline: *hl::translate → parse → compress →
verify → optimize → emit*, reachable both through the integration tests and
...
cargo's `[env]` table does **not** reach external subcommands, and per
Jonathan's preference no machine-level config *files* are used). The
...
State is **not** stored at interior nodes. Each *live leaf* carries a cached
...
additionally record the *current* instruction's own bytes (via
...
are *not* recorded: they target heap/MMIO, not generated storage (soundness of
...
invisible to the transition records, so the node must keep its *original*
...
`MemoryPtr`/`NonNullMemoryPtr` are domain value types, *not*
...
- **Machine state** is `State { registers: Vec<RegisterValues>, memory,
  ...
  *recording* itself goes through the free functions
...
The *type-number* is `FlatType as u64` ([src/ast.rs:279](src/ast.rs#L279)):
...
That generated `src/main.rs` *is* the build: `cargo run` reads `main.hl`, calls
...
C maps near one-to-one onto assembly: every *simple* statement lowers to
exactly one dialect line, and the three *structured* statements (`if`, `while`,
...
branch guarding each `#!`/loop is taken on the *success* path, a proven
...
- `racy_store_annotated`: same program as `racy_store_inferred` but with explicit `#$ value global u32`, so the
  annotation is *checked*, never searched. Asserts the **full 67-step trace**.
...
≈ 6·10¹¹: feasible. The design lever is always *reducing the exponents*, not
...
- **`hl` dispatch order.** In `statement`, assignments are matched *before*
...
- **`accessed`/`transitions` are bookkeeping, never control flow.** The
  ...
  symbolic offset span, record at *every* application site, and never branch
...
ordinary global memory is still *physically* racy; the non-racy treatment is
...
lives at a *symbolic* address; an access either provably stays inside it (so
