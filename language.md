...
### Ruuning assembly: QEMU

QEMU can be installed natively on windows so you don't neccessarily need WSL.
In either case use `qemu-system-riscv64`.
...
- `typed`: Requires all variables have types which can be immedately inferred, removing `8^v` from the onotation. For example `define x local u32` `y=x` works becuase `x` is fully defined and `y` is identical to `x` (this makes type inference as good/bad as a typical language like Rust).
